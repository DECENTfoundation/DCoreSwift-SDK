...
public final class BroadcastApi: BaseApi {
    ...
    public func broadcast(using keypair: ECKeyPair, operations: [BaseOperation], expiration: Int? = nil) -> Completable {
    ...
    public func broadcast(using keypair: String, operations: [BaseOperation], expiration: Int? = nil) -> Completable {
        return Single.just(keypair.chain.keyPair).flatMapCompletable { [unowned self] in
            guard let kp = $0 else { return Completable.error(ChainException.unexpected("Can't create keypair from \(keypair)"))}
    ...
    public func broadcast(withCallback keypair: ECKeyPair, operations: [BaseOperation], expiration: Int? = nil) -> Observable<TransactionConfirmation> {
        return api.transaction.createTransaction(operations, expiration: expiration ?? self.api.transactionExpiration)
            .map { try $0.with(signature: keypair) }
            .asObservable()
    ...
    public func broadcast(withCallback keypair: ECKeyPair, operation: BaseOperation, expiration: Int? = nil) -> Observable<TransactionConfirmation> {
        return broadcast(withCallback: keypair, operations: [operation], expiration: expiration)
    }
    
    public func broadcast(withCallback keypair: String, operations: [BaseOperation], expiration: Int? = nil) -> Observable<TransactionConfirmation> {
        return Single.just(keypair.chain.keyPair).asObservable().flatMap({ kp -> Observable<TransactionConfirmation> in
            guard let kp = kp else { return Observable.error(ChainException.unexpected("Can't create keypair from \(keypair)"))}
            return self.broadcast(withCallback: kp, operations: operations, expiration: expiration)
        })
    }
    
    public func broadcast(withCallback keypair: String, operation: BaseOperation, expiration: Int? = nil) -> Observable<TransactionConfirmation> {
        return broadcast(withCallback: keypair, operations: [operation], expiration: expiration)
    }
