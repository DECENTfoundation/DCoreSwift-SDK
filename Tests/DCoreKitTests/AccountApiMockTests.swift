...
class AccountApiMockTests: MockTestCase {
    ...
    func testGetAccountByIdUsingRest() {
        ...
        let account = try? rest.account.get(byId: "1.2.35").debug().toBlocking().single()
        XCTAssertEqual(account?.id, "1.2.35".dcore.objectId())
    }
