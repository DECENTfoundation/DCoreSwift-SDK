...
extension BlockData: DataConvertible {
    public func asData() -> Data {
        ...
        data += Data(Data(count: 2).indices.map({ UInt8.with(value: UInt64(refBlockNum) >> (8 * $0)) }))
        data += Data(Data(count: 4).indices.map({ UInt8.with(value: refBlockPrefix >> (8 * UInt64($0))) }))
        data += Data(Data(count: 4).indices.map({ UInt8.with(value: expiration >> (8 * UInt64($0))) }))
        
        DCore.Logger.debug(crypto: "BlockData binary: %{private}s", args: {
