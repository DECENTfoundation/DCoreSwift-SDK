...
public final class ContentApi: BaseApi {
    ...
    public func getContent(byId id: ChainObject) -> Single<Content> {
        return GetContentById(id).base.asCoreRequest(api.core).map({ $0.first! })
    }

    public func getContent(byUri uri: String) -> Single<Content> {
        return GetContentByUri(uri).base.asCoreRequest(api.core)
    }
    ...
    public func search(contentByTerm term: String,
        ...
        return SearchContent(term, order: order, user: user, regionCode: regionCode, type: type, startId: startId, limit: limit).base.asCoreRequest(api.core)
    }
    
    public func listPublishingManagers(lowerBound: String, limit: Int = 100) -> Single<[ChainObject]> {
        return ListPublishingManagers(lowerBound, limit: limit).base.asCoreRequest(api.core)
    }
    
    public func generateContentKeys(forSeeders ids: [ChainObject]) -> Single<ContentKeys> {
        return GenerateContentKeys(ids).base.asCoreRequest(api.core)
    }
    
    public func restoreEncryptionKey(elGamalPrivate: PubKey, purchaseId: ChainObject) -> Single<String> {
        return RestoreEncryptionKey(elGamalPrivate, purchaseId: purchaseId).base.asCoreRequest(api.core)
    }
}
