...
extension ProcessedTransaction: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        let results =       try container.decodeIfPresent([AnyValue].self, forKey: .opResults)
        opResults = results?.map {
            if case let .array(items) = $0, items.count >= 2 {
                if case let .string(id) = items[1] {
                    return id.dcore.objectId()
                }
            }
            return nil
        } ?? []
